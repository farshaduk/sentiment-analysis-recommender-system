...
| Area | What We Built | Result |
|:---:|:---|:---:|
| 🧠 **Sentiment Analysis** | VADER + TextBlob lexicon models | VADER wins |
| 🤖 **Machine Learning** | TF-IDF + Logistic Regression + Linear SVM | SVM is best classifier |
| 🛒 **Recommender System** | Sentiment-enhanced collaborative filtering | RMSE ↓ **24.9%** |
| 📝 **LLM Tasks** | Review summarisation + CSR response generation | BART + FLAN-T5 |
| 📊 **Dataset** | Amazon Industrial & Scientific reviews | **77,071** reviews |
...
```mermaid
flowchart TD
    A[📦 Raw Amazon Reviews\nJSON / JSON.GZ] --> B[🔎 Data Exploration\n77,071 Reviews]
    B --> C[🧹 Pre-processing\nClean · Label · Sample]
    ...
    G --> I[🛒 Sentiment-Enhanced\nCollaborative Filtering]
    G --> J[📝 LLM Tasks\nSummariser + CSR Response]
...
> ⚠️ **Class Imbalance Note:** The dataset is heavily skewed toward 5-star reviews. Because of this, **Macro F1** is used as the main evaluation metric — it treats each sentiment class equally regardless of frequency.
...
- TextBlob is more conservative and often over-predicts Neutral
...
```mermaid
flowchart TD
    A[📝 Review Text] --> B[🔢 TF-IDF Vectorizer\nUnigrams + Bigrams\nmin_df=3 · max_features=10K]
    B --> C[📈 Logistic Regression\nC=1.0 · max_iter=1000]
    B --> D[⚔️ Linear SVM\nC=0.5 · dual=False]
    C --> E[📊 Evaluation on 1,000-review test set]
    D --> E
    E --> F[🏆 Linear SVM Selected as Best Model]

    style A fill:#E3F2FD,stroke:#1565C0
    style B fill:#FFF8E1,stroke:#F9A825
    style C fill:#E8F5E9,stroke:#2E7D32
    style D fill:#FCE4EC,stroke:#AD1457
    style F fill:#EDE7F6,stroke:#4527A0,stroke-width:3px
```
...
| 4️⃣ | TextBlob Lexicon | 73.5% | 0.4332 | 0.7854 |

</div>

### 🔑 Key Findings

- ML models outperform lexicon-based methods by **+13–15% on Macro F1**
- The **Neutral** class is the hardest to classify — 3-star reviews contain mixed language
- Linear SVM learns domain-specific negative terms: `broke`, `defect`, `return`
...
The recommender blends star ratings with VADER sentiment scores into item-based collaborative filtering.
...
flowchart LR
    ...
    style F fill:#F3E5F5,stroke:#6A1B9A,stroke-width:3px
```
...
> Blending sentiment scores provides a smoother and more nuanced signal — especially valuable for products where star ratings alone do not fully explain user satisfaction.
...
> ⚠️ First run downloads HuggingFace model weights — approximately **250 MB to 1.6 GB**. Later runs use the local cache at `~/.cache/huggingface/`.
...
| TextBlob Lexicon | 73.5% | 0.4332 | 0.7854 | More conservative |
...
| **Improvement** | **+6.7%** | **+24.9%** | 🚀 Strong gain |
...
> **Important:** Run **Phase 1 before Phase 2** — `sentiment_analysis_results.csv` is required as input for Step 14.
